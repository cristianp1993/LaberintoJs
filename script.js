...
document.getElementById('btn-revelar')
    .addEventListener('click', encontrarCamino(arrayLaberinto), false);
...
function encontrarCamino(laberinto) {
    ...
    const visitados = Array(laberinto.length).fill().map(() => Array(laberinto[0].length).fill(false)); 
    ...
    cola.push({ fila: filaInicial, columna: columnaInicial, distancia: 0, ruta: [] }); 
    ...
    while (cola.length > 0) {
      ...
      for (const vecino of vecinos) {
        ...
        if (!visitados[filaVecino][columnaVecino]) {
          ...
          cola.push({ fila: filaVecino, columna: columnaVecino, distancia: distancia + 1, ruta: [...ruta, { fila: filaVecino, columna: columnaVecino }] }); // Agregar el vecino a la cola con su distancia y ruta
        }
      }
    }
...
// Llamada a la función y mostrar el camino más corto
const caminoMasCorto = encontrarCamino(laberinto);
